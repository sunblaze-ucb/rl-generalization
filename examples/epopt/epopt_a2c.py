...
class EPOptRunner(BaseRunner):
    ...
    def run(self, *,
        ...
        if self.discrete:
            ...
            mb_actions = mb_actions.reshape((mb_rewards.shape[0], self.ac_space.shape[0]))
...
def learn(policy, env,
    ...
    make_model = lambda: EPOptModel(policy=policy, ob_space=ob_space, ac_space=ac_space, nenvs=nenvs, nsteps=nsteps, ent_coef=ent_coef, vf_coef=vf_coef,
